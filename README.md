...
- `setup.sh` installs the herdr terminal workspace manager (https://herdr.dev, AI-agent pane multiplexer) alongside omp: the omp/claude agent integrations feed the sidebar real agent state, `config/herdr/config.toml` is symlinked into `~/.config/herdr/` (onboarding skipped, login-shell panes on macOS), the herdr agent skill lands in the omp/claude skill dirs, and zsh completions are sourced straight from the binary. The binary self-updates via `herdr update`.
